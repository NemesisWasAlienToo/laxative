...
| **Export / import** | JSON that round-trips, including which note file each note came from — an import offers to put them back in those files and creates any that are missing. Markdown for reading and sharing. |
| **Notes with no location** | A note need not be about a line of code. The **+** on the Notes view, **Laxative: Add Note** or `Ctrl+Alt+N` writes one about the workspace itself — a decision, a question, a plan. It is linked, tagged, searched, graphed and exported like any other; it simply points nowhere. There is no flag for it: a note has a location when it carries all three of file, line and character, and anything less is a note about no particular line. |
...
| **Central list** | The **Laxative** activity-bar view lists every note in the workspace, grouped by file, hashtag or note file, with notes that point at no line of code gathered under **no location**. What a click does is up to you — see `laxative.listClickAction`. |
| **Graph** | A **Note Graph** tab in the bottom panel, next to Terminal, the way GitLens puts its commit graph. Obsidian-style force-directed map: arrows are references, faint lines join notes sharing a file or a tag, and node size grows with how connected a note is. Drag to rearrange, double-click to open, filter to hide everything else. **Right-click a note** to open it, go to its code, edit it, copy a reference or delete it; **right-click the background** for the options and for laying the notes out again. Move a note by hand and the layout holds every note where you left it rather than pulling it back to the middle, so clusters can be parked apart. |
...
With more than one file switched on, **adding a note asks which file it belongs
to** — that choice is the point of having several. The likely answer is listed
first (the file you used last, or `laxative.defaultNoteFile` if you pin one), so
